...
interface SearchCatalogEntry {
  ...
  /** Anchor terms generated by offline divergent expansion (v3+). */
...
/** Anchor inverted index: term → list of memory IDs that have this anchor. */
...
interface SearchIndex {
  ...
  /** Inverted index of anchor terms → memory IDs (v3+). Empty array if no anchors exist. */
  readonly anchorIndex: readonly Readonly<AnchorIndexEntry>[];
}

/** Legacy v2 search index (no anchor support). Used only for migration. */
...
export class PersonalMemoryRepository {
  ...
  private async readSearchIndexSnapshot(
    ...
    const raw = await readJsonOrNull<SearchIndex | SearchIndexV2>(this.layout.searchIndex);

    // --- Schema migration: v2 → v3 ---
    // When encountering a legacy v2 index, rebuild as v3 with empty anchorIndex.
    // The anchor data will be populated by the next dreaming cycle.
    if (raw && (raw as unknown as Record<string, unknown>).schemaVersion === 2) {
      const v2 = raw as SearchIndexV2;
      const migrated: SearchIndex = Object.freeze({
        schemaVersion: 3,
        revision: v2.revision,
        catalog: Object.freeze(v2.catalog.map((entry) => Object.freeze({ ...entry }))),
        checksum: v2.checksum,
        anchorIndex: Object.freeze([])
      });
      // Persist the migrated index so we don't re-migrate on every read
      await atomicWrite(this.layout.searchIndex, jsonText(migrated));
      return migrated;
    }

    const index = raw as SearchIndex | null;
    if (
      !index
      ...
      || !Array.isArray(index.anchorIndex)
    ) {
    ...
    return Object.freeze({
      ...
      catalog: Object.freeze(index.catalog.map((record) => Object.freeze({
        ...
        ...(record.anchorTokens ? { anchorTokens: Object.freeze([...record.anchorTokens]) } : {})
      }))),
      anchorIndex: Object.freeze(index.anchorIndex.map((entry) => Object.freeze({
        term: entry.term,
...
function buildSearchIndex(manifest: PersonalMemoryManifest, records: readonly PersonalMemoryRecord[]): SearchIndex {
  const catalog = records
    .map((record) => {
      const anchorTerms = (record.anchors ?? [])
        .filter((a) => a.confidence >= MEMORY_ANCHOR_MIN_CONFIDENCE)
        .map((a) => a.term);
      const entry: SearchCatalogEntry = Object.freeze({
        id: record.id,
        kind: record.kind,
        status: record.status,
        title: record.title,
        ...
        ...(anchorTerms.length > 0
          ? { anchorTokens: Object.freeze(lexicalTokens(anchorTerms.join("\n"))) }
          : {})
      });
      return entry;
    })
  ...
  // Build anchor inverted index: term → [memoryId, ...]
  const anchorMap = new Map<string, string[]>();
  for (const entry of catalog) {
    if (entry.anchorTokens) {
      for (const token of entry.anchorTokens) {
        const existing = anchorMap.get(token);
        if (existing) {
          existing.push(entry.id);
        } else {
          anchorMap.set(token, [entry.id]);
        }
      }
    }
  }
  const anchorIndex: readonly AnchorIndexEntry[] = Object.freeze(
    [...anchorMap.entries()]
      .sort(([a], [b]) => a.localeCompare(b))
      .map(([term, memoryIds]) => Object.freeze({
        term,
        memoryIds: Object.freeze(memoryIds.sort())
      }))
  );
...
function catalogLexicalScore(
  ...
  const anchorTokens = new Set(record.anchorTokens ?? []);
  let score = 0;
  for (const token of queryTokens) {
    if (routeTokens.has(token)) score += token.length > 1 ? 12 : 3;
    else if (anchorTokens.has(token)) score += token.length > 1 ? 8 : 2;

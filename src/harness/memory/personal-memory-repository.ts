...
interface SearchIndex {
  ...
  readonly revision: number;
  readonly catalog: readonly Readonly<SearchCatalogEntry>[];
...
export class PersonalMemoryRepository {
  ...
  private async searchSnapshot(
    ...
    const ranked = index.catalog
      ...
      .map((record) => ({ record, score: catalogLexicalScore(record, query, queryTokens) }))
    ...
    const items = await Promise.all(selected.map(async ({ record, score }) => {
  ...
  private async closeRecord(
    ...
    next.updatedAt = this.now();
    next.records = allRecords.map(recordMetadata);
    const changes = await this.stateChanges(next, allRecords, [{ relativePath: closed.file, content: serializeRecord(closed) }], {
      type: "closed",
      revision: targetRevision,
      at: this.now(),
      recordId: targetId,
  ...
  private async reconcileMarkdownTruth(): Promise<void> {
    ...
    if (!current) {
      ...
      await atomicWrite(this.layout.searchIndex, jsonText(buildSearchIndex(manifest, markdownRecords)));
  ...
  private async stateChanges(
    ...
    return dedupeChanges([
      ...
      { relativePath: path.relative(this.layout.root, this.layout.searchIndex), content: jsonText(buildSearchIndex(manifest, records)) },
  ...
  private async repairDerivedFiles(manifest: PersonalMemoryManifest, records: readonly PersonalMemoryRecord[]): Promise<void> {
    ...
    const expectedIndex = buildSearchIndex(manifest, records);
  ...
  private async readSearchIndexSnapshot(
    ...
    if (
      ...
      || index.checksum !== contentHash(stableJson({ revision: index.revision, catalog: index.catalog }))
    ) {
      throw new PersonalMemoryAccessError("invalid_request", "Personal Memory search-index snapshot is invalid");
    }
    ...
    if (
      ...
      throw new PersonalMemoryAccessError("invalid_request", "Personal Memory search-index contains ghost records");
    }
    return Object.freeze({
      ...index,
...
function buildSearchIndex(manifest: PersonalMemoryManifest, records: readonly PersonalMemoryRecord[]): SearchIndex {
  ...
  const checksum = contentHash(stableJson({ revision: manifest.revision, catalog }));
  return {
    schemaVersion: 2,
    revision: manifest.revision,
...
function lexicalTokens(text: string): string[] {
  ...
  for (const match of normalized.matchAll(/[a-z0-9_]+/gu)) tokens.add(match[0]);
  for (const match of normalized.matchAll(/\p{Script=Han}/gu)) tokens.add(match[0]);
  return [...tokens].sort(compareText);
}

function lexicalScore(record: PersonalMemoryRecord, query: string, queryTokens: readonly string[]): number {
  if (!queryTokens.length) return 1;
  const haystack = `${record.title}\n${record.content}\n${record.scope ?? ""}`.normalize("NFKC").toLocaleLowerCase();
  const tokens = new Set(lexicalTokens(haystack));
  let score = 0;
  for (const token of queryTokens) if (tokens.has(token)) score += token.length > 1 ? 4 : 1;
  if (query.trim() && haystack.includes(query.trim().normalize("NFKC").toLocaleLowerCase())) score += 12;
  if (record.status === "current") score += 1;
  return score;
}

function catalogLexicalScore(
  record: Readonly<SearchCatalogEntry>,
  query: string,
  queryTokens: readonly string[]
): number {
  if (!queryTokens.length) return 1;
  const routeTokens = new Set(record.routeTokens);
  const contentTokens = new Set(record.contentTokens);
  let score = 0;
  for (const token of queryTokens) {
    if (routeTokens.has(token)) score += token.length > 1 ? 12 : 3;
    else if (contentTokens.has(token)) score += token.length > 1 ? 2 : 0.5;
  }
  const route = `${record.title}\n${record.recallWhen}\n${record.scope ?? ""}`
    .normalize("NFKC")
    .toLocaleLowerCase();
  for (const phrase of normalizedQueryPhrases(query)) {
    if (route.includes(phrase)) score += 24;
  }
  if (record.status === "current") score += 1;
  return score;
}

function normalizedQueryPhrases(query: string): string[] {
  return [...new Set(query
    .split(/\r?\n/gu)
    .map((line) => line.trim().normalize("NFKC").toLocaleLowerCase())

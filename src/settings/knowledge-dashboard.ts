...
export function renderSettingsKnowledgeDashboard(
  ...
  const refresh = button(controls, t("刷新状态", "Refresh status"), state.loading ? "loader-circle" : "refresh-cw", actions.onRefresh);
  refresh.disabled = state.loading;
  const toggle = button(controls, state.expanded ? t("收起详情", "Collapse details") : t("展开详情", "Expand details"), state.expanded ? "chevron-up" : "chevron-down", actions.onToggleExpanded);
  ...
  label.createSpan({ cls: `status ${health.status === "healthy" ? "healthy" : "neutral"}`, text: healthLabel });
  ...
  check.createDiv({ cls: "card-label" }).createEl("h3", { text: t("体检新鲜度", "Check freshness") });
  check.createDiv({ cls: "check-empty-title" }).createEl("strong", { text: freshness.lastCheckAt
  ...
  check.createDiv({ cls: "check-note", text: t("维护成功不等于完成全库体检。", "Successful maintenance is not a full knowledge-base check.") });
...
function button(container: HTMLElement, label: string, icon: string, onClick: () => void): HTMLButtonElement {
  const el = container.createEl("button", { cls: "text-button", attr: { type: "button", "aria-label": label } });
  setIcon(el.createSpan(), icon); el.createSpan({ text: label }); el.onclick = onClick;
  return el;
}
...
function table(container: HTMLElement, title: string, total: number, columns: string[]): HTMLTableSectionElement {
  ...
  heading.createEl("h3", { text: title }); heading.createSpan({ cls: "panel-total", text: String(total) });

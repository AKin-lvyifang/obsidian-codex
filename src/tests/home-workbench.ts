...
function assertHomeGraphAndMagicUiContracts(): void {
  ...
  assert.match(view, /echoink-home-graph-related-row is-current is-note[\s\S]*text: `当前：\$\{formatGraphRelationLabel\(node\.cluster, node\.title\)\}`[\s\S]*renderGraphPopoutButton\(current, node\.id, node\.title\)/u);
  ...
  assert.match(view, /createEl\("li", \{ cls: "is-note" \}\)[\s\S]*formatGraphRelationLabel\(itemData\.detail, itemData\.title\)[\s\S]*focus\.onclick = \(\) => this\.graphController\.focusNode\(itemData\.noteId\)[\s\S]*echoink-home-graph-related-cluster[\s\S]*"aria-hidden": "true"[\s\S]*renderGraphPopoutButton\(item, itemData\.noteId, itemData\.title\)/u);
  ...
  assert.match(styles, /\.echoink-home-graph-related\.is-note\s*\{[^}]*margin-top: 19px;/u);
  assert.match(styles, /\.echoink-home-graph-related ul\s*\{[^}]*gap: 2px;/u);
  assert.match(styles, /\.echoink-home-graph-related li\.is-note\s*\{[^}]*min-height: 48px;[^}]*grid-template-columns: minmax\(0, 1fr\) clamp\(56px, 24%, 72px\) 28px;[^}]*margin-inline-start: 16px;/u);
  assert.match(styles, /\.echoink-home-graph-related-row\.is-note\s*\{[^}]*grid-template-columns: minmax\(0, 1fr\) 28px;/u);
  ...
  assert.match(styles, /\.echoink-home-graph-related li\.is-note > \.echoink-home-graph-related-focus\s*\{[^}]*font-size: 14px;[^}]*line-height: 20px;/u);
  assert.match(styles, /\.echoink-home-graph-related-row\.is-current \.echoink-home-graph-related-focus\s*\{[^}]*font-size: 13px;[^}]*line-height: 20px;/u);
  assert.match(styles, /\.echoink-home-graph-related-cluster\s*\{[^}]*overflow: hidden;[^}]*font-size: 13px;[^}]*line-height: 20px;[^}]*text-align: end;[^}]*text-overflow: ellipsis;[^}]*white-space: nowrap;/u);
  assert.match(styles, /\.echoink-home-graph-related \.echoink-home-graph-related-open,[\s\S]*min-width: 28px;/u);
  assert.match(styles, /@container echoink-home \(max-width: 520px\)[\s\S]*\.echoink-home-graph-related li\.is-note\s*\{[^}]*grid-template-columns: minmax\(0, 1fr\) 28px;[^}]*margin-inline-start: 0;[^}]*\}[\s\S]*\.echoink-home-graph-related li\.is-note \.echoink-home-graph-related-cluster\s*\{[^}]*display: none;/u);
